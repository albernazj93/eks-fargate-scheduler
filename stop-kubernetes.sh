handler () {
    ...
    for deploy in $(kubectl get deployments -n dev | tail | cut -d ' ' -f 1); do \
        replicas=$(kubectl get deployment/$deploy -n dev -o jsonpath='{@.spec.replicas}'); \
        kubectl annotate --overwrite deployment/$deploy previous-replicas=$replicas -n dev; \
        kubectl scale --replicas 0 deployment/$deploy -n dev; \
    done
